...
struct PipelineStageInfo {
    ...
    /// When false, the compiled MSL omits `_buffer_sizes` for this stage; skip
    /// `setVertexBytes` / `setFragmentBytes` (etc.) for [`Self::sizes_slot`].
...
impl PipelineStageInfo {
    ...
    fn assign_from(&mut self, other: &Self) {
        ...
        self.upload_sizes_bytes = other.upload_sizes_bytes;
        self.sized_bindings.clear();
...
pub struct RenderPipeline {
    ...
    /// `setVertexBuffer` indices `0..vs_layout_buffer_count` are reserved for this pipeline's
    /// layout (immediates, bind groups, sizes). Used to clear stale low slots on PSO change.
    vs_layout_buffer_count: u32,
    /// Same for the fragment stage (`setFragmentBuffer` table).
    fs_layout_buffer_count: u32,
    vs_layout_texture_count: u32,
    fs_layout_texture_count: u32,
    vs_layout_sampler_count: u32,
    fs_layout_sampler_count: u32,
    /// Merged bitmask: MSL `[[buffer(i)]]` scan OR every non-`sizes_buffer` layout slot. Used for
    /// `setVertexBuffer` / bind groups and for nil-ing stale low-table slots (scan can miss real
    /// bind-group buffers).
    vs_metal_buffer_bindings_mask: Option<u32>,
    fs_metal_buffer_bindings_mask: Option<u32>,
    /// MSL scan only (no layout OR). Used to gate `setVertexBytes` / `_buffer_sizes` so we do not
    /// upload bytes at indices the entry point does not declare.
    vs_metal_buffer_bindings_mask_parsed: Option<u32>,
    fs_metal_buffer_bindings_mask_parsed: Option<u32>,
    vs_metal_texture_bindings_mask: Option<u128>,
    fs_metal_texture_bindings_mask: Option<u128>,
    vs_metal_sampler_bindings_mask: Option<u128>,
    fs_metal_sampler_bindings_mask: Option<u128>,
...
/// Metal argument table sizes for one shader stage (non–argument-buffer tier-2 path).
pub(super) const METAL_MAX_LOW_TABLE_BUFFERS: usize = 31;
pub(super) const METAL_MAX_STAGE_SAMPLERS: usize = 16;
pub(super) const METAL_MAX_STAGE_TEXTURES: usize = 128;

/// Tracks last-bound pointers/offsets so we can skip redundant `setSamplerState` /
/// `setTexture` / `setBuffer` calls (Metal debug validation logs each redundant set).
#[derive(Clone)]
pub(super) struct MetalStageArgumentCache {
    pub samplers: [Option<usize>; METAL_MAX_STAGE_SAMPLERS],
    pub textures: [Option<usize>; METAL_MAX_STAGE_TEXTURES],
    pub buffers: [Option<(usize, usize)>; METAL_MAX_LOW_TABLE_BUFFERS],
}

impl Default for MetalStageArgumentCache {
    fn default() -> Self {
        Self {
            samplers: [None; METAL_MAX_STAGE_SAMPLERS],
            textures: [None; METAL_MAX_STAGE_TEXTURES],
            buffers: [None; METAL_MAX_LOW_TABLE_BUFFERS],
        }
    }
}
...
struct CommandState {
    ...
    /// Metal vertex buffer argument indices with an active `setVertexBuffer`
    /// binding in this render pass. Used to clear stale slots when switching
    /// to a pipeline that uses fewer vertex buffers (satisfies Metal validation).
    bound_vertex_metal_indices: hashbrown::HashSet<u64>,

    /// Last bound render pipeline layout buffer counts (low table indices per stage).
    vs_layout_buffer_count: u32,
    fs_layout_buffer_count: u32,

    /// Merged VS/FS buffer mask (MSL scan OR layout); see [`RenderPipeline::vs_metal_buffer_bindings_mask`].
    vs_metal_buffer_bindings_mask: Option<u32>,
    fs_metal_buffer_bindings_mask: Option<u32>,
    /// Parsed-only buffer mask for inline `set*Bytes` (push constants, `_buffer_sizes`).
    vs_metal_buffer_bindings_mask_parsed: Option<u32>,
    fs_metal_buffer_bindings_mask_parsed: Option<u32>,
    vs_metal_texture_bindings_mask: Option<u128>,
    fs_metal_texture_bindings_mask: Option<u128>,
    ...
    /// Last fixed-function state on the active render encoder (skip redundant sets).
    render_front_winding: Option<MTLWinding>,
    render_cull_mode: Option<MTLCullMode>,
    render_triangle_fill_mode: Option<MTLTriangleFillMode>,
    render_depth_clip_mode: Option<MTLDepthClipMode>,
    render_depth_stencil_ptr: Option<usize>,
    render_depth_bias_key: Option<(i32, u32, u32)>,

    vs_arg_cache: MetalStageArgumentCache,
    fs_arg_cache: MetalStageArgumentCache,
    ts_arg_cache: MetalStageArgumentCache,
    ms_arg_cache: MetalStageArgumentCache,
    cs_arg_cache: MetalStageArgumentCache,

    /// Last vertex-stream `setVertexBuffer` (Metal buffer index → object id, offset).
    vertex_stream_binding_cache: [Option<(usize, usize)>; METAL_MAX_LOW_TABLE_BUFFERS],
