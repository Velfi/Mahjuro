...
impl Default for super::CommandState {
    fn default() -> Self {
        Self {
            ...
            bound_vertex_metal_indices: Default::default(),
            vs_layout_buffer_count: 0,
            fs_layout_buffer_count: 0,
            vs_metal_buffer_bindings_mask: None,
            fs_metal_buffer_bindings_mask: None,
            vs_metal_buffer_bindings_mask_parsed: None,
            fs_metal_buffer_bindings_mask_parsed: None,
            vs_metal_texture_bindings_mask: None,
            fs_metal_texture_bindings_mask: None,
            vs_metal_sampler_bindings_mask: None,
            fs_metal_sampler_bindings_mask: None,
            immediates: Vec::new(),
            render_front_winding: None,
            render_cull_mode: None,
            render_triangle_fill_mode: None,
            render_depth_clip_mode: None,
            render_depth_stencil_ptr: None,
            render_depth_bias_key: None,
            vs_arg_cache: Default::default(),
            fs_arg_cache: Default::default(),
            ts_arg_cache: Default::default(),
            ms_arg_cache: Default::default(),
            cs_arg_cache: Default::default(),
            vertex_stream_binding_cache: [None; super::METAL_MAX_LOW_TABLE_BUFFERS],
...
impl super::CommandEncoder {
    ...
    fn update_bind_group_state(
        ...
        let metal_buffer_mask = match encoder.stage() {
            S::Vertex => self.state.vs_metal_buffer_bindings_mask,
            S::Fragment => self.state.fs_metal_buffer_bindings_mask,
            S::Task | S::Mesh | S::Compute => None,
            S::RayGeneration | S::AnyHit | S::ClosestHit | S::Miss => unimplemented!(),
        };
        let metal_texture_mask = match encoder.stage() {
            S::Vertex => self.state.vs_metal_texture_bindings_mask,
            S::Fragment => self.state.fs_metal_texture_bindings_mask,
            S::Task | S::Mesh | S::Compute => None,
            S::RayGeneration | S::AnyHit | S::ClosestHit | S::Miss => unimplemented!(),
        };
        let metal_sampler_mask = match encoder.stage() {
            S::Vertex => self.state.vs_metal_sampler_bindings_mask,
            S::Fragment => self.state.fs_metal_sampler_bindings_mask,
            S::Task | S::Mesh | S::Compute => None,
            S::RayGeneration | S::AnyHit | S::ClosestHit | S::Miss => unimplemented!(),
        };
        let mut changes_sizes_buffer = false;
        for index in 0..buffers {
            let arg_cache = match encoder.stage() {
                S::Vertex => &mut self.state.vs_arg_cache,
                S::Fragment => &mut self.state.fs_arg_cache,
                S::Task => &mut self.state.ts_arg_cache,
                S::Mesh => &mut self.state.ms_arg_cache,
                S::Compute => &mut self.state.cs_arg_cache,
                S::RayGeneration | S::AnyHit | S::ClosestHit | S::Miss => unimplemented!(),
            };
            ...
            match res {
                super::BufferLikeResource::Buffer {
                    ...
                    if index < 32 {
                        if let Some(m) = metal_buffer_mask {
                            if m & (1u32 << index) == 0 {
                                if index < super::METAL_MAX_LOW_TABLE_BUFFERS {
                                    arg_cache.buffers[index] = None;
                                }
                                encoder.set_buffer(None, 0, index);
                                continue;
                            }
                        }
                    }
                    ...
                    let should_set = if index < super::METAL_MAX_LOW_TABLE_BUFFERS {
                        let key = Some((ptr_id, off));
                        if arg_cache.buffers[index] == key {
                            false
                        } else {
                            arg_cache.buffers[index] = key;
                            true
                        }
                    } else {
                        true
                    ...
                    if should_set {
                        encoder.set_buffer(buffer, off, index);
                    }
                    ...
                    if index < 32 {
                        if let Some(m) = metal_buffer_mask {
                            if m & (1u32 << index) == 0 {
                                ...
                                encoder.set_acceleration_structure(None, index);
                                continue;
                            }
                        }
                    }
        ...
        if changes_sizes_buffer {
            let stage = encoder.stage();
            let (sizes_slot, upload_sz, mask) = match stage {
                S::Vertex => (
                    self.state.stage_infos.vs.sizes_slot,
                    self.state.stage_infos.vs.upload_sizes_bytes,
                    self.state.vs_metal_buffer_bindings_mask_parsed,
                ),
                S::Fragment => (
                    self.state.stage_infos.fs.sizes_slot,
                    self.state.stage_infos.fs.upload_sizes_bytes,
                    self.state.fs_metal_buffer_bindings_mask_parsed,
                ),
                S::Task => (
                    self.state.stage_infos.ts.sizes_slot,
                    self.state.stage_infos.ts.upload_sizes_bytes,
                    None,
                ),
                S::Mesh => (
                    self.state.stage_infos.ms.sizes_slot,
                    self.state.stage_infos.ms.upload_sizes_bytes,
                    None,
                ),
                S::Compute => (
                    self.state.stage_infos.cs.sizes_slot,
                    self.state.stage_infos.cs.upload_sizes_bytes,
                    None,
                ),
            ...
            if allow {
                ...
                } else if let Some(slot) = sizes_slot {
                    // Clear stale *sizes* bytes when this stage's MSL entry actually takes
                    // `_buffer_sizes` at the layout sizes slot but we have nothing to upload.
                    //
                    // When `upload_sizes_bytes` is false, the compiled shader does not declare
                    // `_buffer_sizes` at this index — the slot is part of the bindless table and
                    // may hold a real `[[buffer(i)]]` resource (e.g. `globals[0]` at index 0).
                    // Nil-ing it here aborts Metal validation ("missing Buffer binding …").
                    clear_sizes_slot(self, slot);
                }
            }
        }
        ...
        for index in 0..samplers {
            ...
            if index < 128 {
                if let Some(m) = metal_sampler_mask {
                    if m & (1u128 << index) == 0 {
                        if index < super::METAL_MAX_STAGE_SAMPLERS {
                            arg_cache.samplers[index] = None;
                        }
                        encoder.set_sampler_state(None, index);
                        continue;
                    }
                }
            }
            let ptr_id = res.as_ptr() as usize;
            let should_set = if index < super::METAL_MAX_STAGE_SAMPLERS {
                if arg_cache.samplers[index] == Some(ptr_id) {
                    false
                } else {
                    arg_cache.samplers[index] = Some(ptr_id);
                    true
                }
            } else {
                true
        ...
        for index in 0..textures {
            ...
            if index < 128 {
                if let Some(m) = metal_texture_mask {
                    if m & (1u128 << index) == 0 {
                        if index < super::METAL_MAX_STAGE_TEXTURES {
                            arg_cache.textures[index] = None;
                        }
                        encoder.set_texture(None, index);
                        continue;
                    }
                }
            }
            let ptr_id = res.as_ptr() as usize;
            let should_set = if index < super::METAL_MAX_STAGE_TEXTURES {
                if arg_cache.textures[index] == Some(ptr_id) {
                    false
                } else {
                    arg_cache.textures[index] = Some(ptr_id);
                    true
                }
            } else {
                true
...
impl super::CommandState {
    fn reset(&mut self) {
        ...
        self.bound_vertex_metal_indices.clear();
        self.vs_layout_buffer_count = 0;
        self.fs_layout_buffer_count = 0;
        self.vs_metal_buffer_bindings_mask = None;
        self.fs_metal_buffer_bindings_mask = None;
        self.vs_metal_buffer_bindings_mask_parsed = None;
        self.fs_metal_buffer_bindings_mask_parsed = None;
        self.vs_metal_texture_bindings_mask = None;
        self.fs_metal_texture_bindings_mask = None;
        self.vs_metal_sampler_bindings_mask = None;
        self.fs_metal_sampler_bindings_mask = None;
        ...
        self.render_front_winding = None;
        self.render_cull_mode = None;
        self.render_triangle_fill_mode = None;
        self.render_depth_clip_mode = None;
        self.render_depth_stencil_ptr = None;
        self.render_depth_bias_key = None;
        self.vs_arg_cache = Default::default();
        self.fs_arg_cache = Default::default();
        self.ts_arg_cache = Default::default();
        self.ms_arg_cache = Default::default();
        self.cs_arg_cache = Default::default();
        self.vertex_stream_binding_cache = [None; super::METAL_MAX_LOW_TABLE_BUFFERS];
    }

    fn clear_stage_buffer_slot(cache: &mut super::MetalStageArgumentCache, idx: usize) {
        if idx < super::METAL_MAX_LOW_TABLE_BUFFERS {
            cache.buffers[idx] = None;
        }
    }

    /// Whether to call `set*Bytes` for `_buffer_sizes` at the layout sizes slot.
    ///
    /// Uses the **MSL-parsed** `[[buffer(i)]]` mask only. The merged layout mask must not be used
    /// here: it ORs in every bind-group slot, which falsely marks the sizes index as referenced and
    /// triggers Metal validation (`Set Vertex Bytes Validation` / unused binding).
    fn stage_may_bind_sizes_bytes_at_slot(
        sizes_slot: Option<naga::back::msl::Slot>,
        upload_sizes_bytes: bool,
        msl_entry_buffer_mask_parsed: Option<u32>,
    ) -> bool {
        if !upload_sizes_bytes {
            return false;
        }
        let Some(slot) = sizes_slot else {
            return false;
        };
        let s = slot as u32;
        if s >= 32 {
        ...
        match msl_entry_buffer_mask_parsed {
            Some(mask) => (mask & (1 << s)) != 0,
            // Without an MSL/Naga mask (e.g. some passthrough modules), do not guess — uploading
            // `_buffer_sizes` here when the entry point does not consume them triggers validation.
            None => false,
        }
    }
    ...
    fn make_sizes_buffer_update<'a>(
        ...
        if !stage_info.upload_sizes_bytes {
            return None;
        }
...
impl crate::CommandEncoder for super::CommandEncoder {
    ...
    unsafe fn set_immediates(
        ...
        if let Some(ref render) = self.state.render {
            if let Some(vs) = layout.immediates_infos.vs {
                let upload_vs = match self.state.vs_metal_buffer_bindings_mask_parsed {
                    Some(mask) => {
                        let i = vs.buffer_index;
                        i < 32 && (mask & (1u32 << i)) != 0
                    }
                    None => true,
                };
            ...
            if let Some(fs) = layout.immediates_infos.fs {
                let upload_fs = match self.state.fs_metal_buffer_bindings_mask_parsed {
                    Some(mask) => {
                        let i = fs.buffer_index;
                        i < 32 && (mask & (1u32 << i)) != 0
                    }
                    None => true,
                };
    ...
    unsafe fn set_render_pipeline(&mut self, pipeline: &super::RenderPipeline) {
        let old_vs_sizes_slot = self.state.stage_infos.vs.sizes_slot;
        let old_fs_sizes_slot = self.state.stage_infos.fs.sizes_slot;
        let old_vs_immediates = self.state.stage_infos.vs.immediates;
        let old_fs_immediates = self.state.stage_infos.fs.immediates;
        let old_vs_layout_count = self.state.vs_layout_buffer_count;
        let old_fs_layout_count = self.state.fs_layout_buffer_count;
        ...
        // Metal keeps vertex buffer bindings across `setRenderPipelineState`.
        // If the new VS uses fewer vertex buffer slots than the previous
        // pipeline, leftover bindings trigger debug validation ("unused binding
        // at Buffer index …"). Unbind with `nil` per Apple docs.
        ...
        let stale: Vec<u64> = self
            .state
            .bound_vertex_metal_indices
            .difference(&active_vertex_slots)
            .copied()
            .collect();
        for metal_idx in stale {
            let i = metal_idx as usize;
            unsafe {
                encoder.setVertexBuffer_offset_atIndex(None, 0, i);
            }
            if i < super::METAL_MAX_LOW_TABLE_BUFFERS {
                self.state.vertex_stream_binding_cache[i] = None;
            }
            self.state.bound_vertex_metal_indices.remove(&metal_idx);
            self.state.vertex_buffer_size_map.remove(&metal_idx);
        }

        let new_vs_layout_count = pipeline.vs_layout_buffer_count;
        if old_vs_layout_count > new_vs_layout_count {
            for idx in new_vs_layout_count..old_vs_layout_count {
                let i = idx as usize;
                unsafe {
                    encoder.setVertexBuffer_offset_atIndex(None, 0, i);
                }
                super::CommandState::clear_stage_buffer_slot(&mut self.state.vs_arg_cache, i);
            }
        }

        let new_fs_layout_count = pipeline.fs_layout_buffer_count;
        if old_fs_layout_count > new_fs_layout_count {
            for idx in new_fs_layout_count..old_fs_layout_count {
                let i = idx as usize;
                unsafe {
                    encoder.setFragmentBuffer_offset_atIndex(None, 0, i);
                }
                super::CommandState::clear_stage_buffer_slot(&mut self.state.fs_arg_cache, i);
            }
        }

        // `setVertexBytes` for the VS sizes buffer shares the same argument table
        // as `setVertexBuffer`. When the new pipeline uses a different sizes slot
        // (or none), Metal validation reports an unused binding at the old index.
        let new_vs_sizes_slot = pipeline.vs_info.as_ref().and_then(|i| i.sizes_slot);
        if let Some(old) = old_vs_sizes_slot {
            if new_vs_sizes_slot != Some(old) {
                let i = old as usize;
                unsafe {
                    encoder.setVertexBuffer_offset_atIndex(None, 0, i);
                }
        ...
        let new_fs_sizes_slot = pipeline.fs_info.as_ref().and_then(|i| i.sizes_slot);
        if let Some(old) = old_fs_sizes_slot {
            if new_fs_sizes_slot != Some(old) {
                let i = old as usize;
                unsafe {
                    encoder.setFragmentBuffer_offset_atIndex(None, 0, i);
                }
        ...
        let new_vs_imm = pipeline.vs_info.as_ref().and_then(|i| i.immediates);
        if let Some(old_imm) = old_vs_immediates {
            if new_vs_imm.map(|n| n.buffer_index) != Some(old_imm.buffer_index) {
                let i = old_imm.buffer_index as usize;
                unsafe {
                    encoder.setVertexBuffer_offset_atIndex(None, 0, i);
                }
                super::CommandState::clear_stage_buffer_slot(&mut self.state.vs_arg_cache, i);
        ...
        let new_fs_imm = pipeline.fs_info.as_ref().and_then(|i| i.immediates);
        if let Some(old_imm) = old_fs_immediates {
            if new_fs_imm.map(|n| n.buffer_index) != Some(old_imm.buffer_index) {
                let i = old_imm.buffer_index as usize;
                unsafe {
                    encoder.setFragmentBuffer_offset_atIndex(None, 0, i);
                }
        ...
        self.state.vs_layout_buffer_count = new_vs_layout_count;
        self.state.fs_layout_buffer_count = new_fs_layout_count;
        self.state.vs_metal_buffer_bindings_mask = pipeline.vs_metal_buffer_bindings_mask;
        self.state.fs_metal_buffer_bindings_mask = pipeline.fs_metal_buffer_bindings_mask;
        self.state.vs_metal_buffer_bindings_mask_parsed =
            pipeline.vs_metal_buffer_bindings_mask_parsed;
        self.state.fs_metal_buffer_bindings_mask_parsed =
            pipeline.fs_metal_buffer_bindings_mask_parsed;
        self.state.vs_metal_texture_bindings_mask = pipeline.vs_metal_texture_bindings_mask;
        self.state.fs_metal_texture_bindings_mask = pipeline.fs_metal_texture_bindings_mask;
        self.state.vs_metal_sampler_bindings_mask = pipeline.vs_metal_sampler_bindings_mask;
        self.state.fs_metal_sampler_bindings_mask = pipeline.fs_metal_sampler_bindings_mask;
        ...
        if let Some(mask) = pipeline.vs_metal_buffer_bindings_mask {
            ...
            for idx in 0..cap {
                if mask & (1u32 << idx) == 0 {
                    unsafe {
                        encoder.setVertexBuffer_offset_atIndex(None, 0, idx);
                    }
                    super::CommandState::clear_stage_buffer_slot(&mut self.state.vs_arg_cache, idx);
                }
            }
        }
        if let Some(mask) = pipeline.fs_metal_buffer_bindings_mask {
            ...
            for idx in 0..cap {
                if mask & (1u32 << idx) == 0 {
                    ...
                    super::CommandState::clear_stage_buffer_slot(&mut self.state.fs_arg_cache, idx);
                }
            }
        }
        ...
        if pipeline.fs_info.is_none() {
            ...
            for idx in 0..ft_lim {
                unsafe {
                    encoder.setFragmentTexture_atIndex(None, idx);
                }
                self.state.fs_arg_cache.textures[idx] = None;
            }
            let fs_lim =
                (pipeline.fs_layout_sampler_count as usize).min(super::METAL_MAX_STAGE_SAMPLERS);
            for idx in 0..fs_lim {
                unsafe {
                    encoder.setFragmentSamplerState_atIndex(None, idx);
                }
                self.state.fs_arg_cache.samplers[idx] = None;
            }
        }
            ...
            if let Some(tex_mask) = pipeline.vs_metal_texture_bindings_mask {
                ...
                for idx in 0..lim {
                    if (tex_mask & (1u128 << idx)) == 0 {
                        unsafe {
                            encoder.setVertexTexture_atIndex(None, idx);
                        }
                        self.state.vs_arg_cache.textures[idx] = None;
                    }
                }
            }
        }
        ...
        if pipeline.vs_info.is_some() {
            if let Some(samp_mask) = pipeline.vs_metal_sampler_bindings_mask {
                let lim = (pipeline.vs_layout_sampler_count as usize)
                    .min(super::METAL_MAX_STAGE_SAMPLERS);
                for idx in 0..lim {
                    if (samp_mask & (1u128 << idx)) == 0 {
                        unsafe {
                            encoder.setVertexSamplerState_atIndex(None, idx);
                        }
                        self.state.vs_arg_cache.samplers[idx] = None;
                    }
                }
            }
        }
        if pipeline.fs_info.is_some() {
            if let Some(samp_mask) = pipeline.fs_metal_sampler_bindings_mask {
                let lim = (pipeline.fs_layout_sampler_count as usize)
                    .min(super::METAL_MAX_STAGE_SAMPLERS);
                for idx in 0..lim {
                    if (samp_mask & (1u128 << idx)) == 0 {
                        unsafe {
        ...
        if self.state.render_front_winding != Some(pipeline.raw_front_winding) {
            encoder.setFrontFacingWinding(pipeline.raw_front_winding);
            self.state.render_front_winding = Some(pipeline.raw_front_winding);
        }
        if self.state.render_cull_mode != Some(pipeline.raw_cull_mode) {
            encoder.setCullMode(pipeline.raw_cull_mode);
            self.state.render_cull_mode = Some(pipeline.raw_cull_mode);
        }
        if self.state.render_triangle_fill_mode != Some(pipeline.raw_triangle_fill_mode) {
            encoder.setTriangleFillMode(pipeline.raw_triangle_fill_mode);
            self.state.render_triangle_fill_mode = Some(pipeline.raw_triangle_fill_mode);
        }
        if let Some(depth_clip) = pipeline.raw_depth_clip_mode {
            if self.state.render_depth_clip_mode != Some(depth_clip) {
                encoder.setDepthClipMode(depth_clip);
                self.state.render_depth_clip_mode = Some(depth_clip);
            }
        }
        if let Some((ref state, bias)) = pipeline.depth_stencil {
            let ptr = Retained::as_ptr(state) as usize;
            if self.state.render_depth_stencil_ptr != Some(ptr) {
                encoder.setDepthStencilState(Some(state));
                self.state.render_depth_stencil_ptr = Some(ptr);
            }
            let bk = (
                bias.constant,
            ...
            if self.state.render_depth_bias_key != Some(bk) {
                encoder.setDepthBias_slopeScale_clamp(
                    bias.constant as f32,
                    bias.slope_scale,
                    bias.clamp,
                );
                self.state.render_depth_bias_key = Some(bk);
            }
        }

        if let Some(ref vi) = pipeline.vs_info {
            let allow_vs_sizes = super::CommandState::stage_may_bind_sizes_bytes_at_slot(
                vi.sizes_slot,
                vi.upload_sizes_bytes,
                pipeline.vs_metal_buffer_bindings_mask_parsed,
            );
            if allow_vs_sizes {
                ...
                } else if let Some(slot) = vi.sizes_slot {
                    let i = slot as usize;
                    unsafe {
                        encoder.setVertexBuffer_offset_atIndex(None, 0, i);
                    }
        ...
        if let Some(ref fi) = pipeline.fs_info {
            let allow_fs_sizes = super::CommandState::stage_may_bind_sizes_bytes_at_slot(
                fi.sizes_slot,
                fi.upload_sizes_bytes,
                pipeline.fs_metal_buffer_bindings_mask_parsed,
            );
            if allow_fs_sizes {
                if let Some((index, sizes)) = self.state.make_sizes_buffer_update(
                ...
                } else if let Some(slot) = fi.sizes_slot {
                    let i = slot as usize;
                    unsafe {
                        encoder.setFragmentBuffer_offset_atIndex(None, 0, i);
                    }
        ...
        if let Some(ts_info) = &pipeline.ts_info {
            ...
            if ts_info.upload_sizes_bytes {
                ...
                } else if let Some(slot) = ts_info.sizes_slot {
                    let i = slot as usize;
                    unsafe {
                        encoder.setObjectBuffer_offset_atIndex(None, 0, i);
                    }
                    super::CommandState::clear_stage_buffer_slot(&mut self.state.ts_arg_cache, i);
                }
            }
        }
        if let Some(ms_info) = pipeline.ms_info.as_ref() {
            ...
            if ms_info.upload_sizes_bytes {
                ...
                } else if let Some(slot) = ms_info.sizes_slot {
                    let i = slot as usize;
                    unsafe {
                        encoder.setMeshBuffer_offset_atIndex(None, 0, i);
                    }
                    super::CommandState::clear_stage_buffer_slot(&mut self.state.ms_arg_cache, i);
                }
            }
        }
    }
    ...
    unsafe fn set_vertex_buffer<'a>(
        ...
        let encoder = self.state.render.as_ref().unwrap();
        let metal_i = buffer_index as usize;
        let ptr_id = core::ptr::from_ref(&*binding.buffer.raw) as usize;
        let off = binding.offset as usize;
        let skip_bind = if metal_i < super::METAL_MAX_LOW_TABLE_BUFFERS {
            if self.state.vertex_stream_binding_cache[metal_i] == Some((ptr_id, off)) {
                true
            } else {
                self.state.vertex_stream_binding_cache[metal_i] = Some((ptr_id, off));
                false
            }
        } else {
            false
        ...
        if allow_vs_sizes {
            ...
            } else if let Some(slot) = self.state.stage_infos.vs.sizes_slot {
                let i = slot as usize;
                unsafe {
                    encoder.setVertexBuffer_offset_atIndex(None, 0, i);
                }
    ...
    unsafe fn set_compute_pipeline(&mut self, pipeline: &super::ComputePipeline) {
        let old_cs_sizes_slot = self.state.stage_infos.cs.sizes_slot;
        let previous_sizes =
        ...
        // `setBytes` for runtime array sizes shares the kernel buffer table with `setBuffer`.
        // Clear the previous slot when it moves (mirrors `set_render_pipeline` VS/FS handling).
        let new_cs_sizes_slot = pipeline.cs_info.sizes_slot;
        if let Some(old) = old_cs_sizes_slot {
            if new_cs_sizes_slot != Some(old) {
                let i = old as usize;
                unsafe {
                    encoder.setBuffer_offset_atIndex(None, 0, i);
                }
                super::CommandState::clear_stage_buffer_slot(&mut self.state.cs_arg_cache, i);
            }
        }

        if pipeline.cs_info.upload_sizes_bytes {
            if let Some((index, sizes)) = self
                .state
            ...
            } else if let Some(slot) = pipeline.cs_info.sizes_slot {
                // Encoder state persists; nil stale *sizes* bytes when this kernel takes
                // `_buffer_sizes` but we have nothing to upload.
                let i = slot as usize;
                unsafe {
                    encoder.setBuffer_offset_atIndex(None, 0, i);
                }
