...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let back_uvw = clamp(cell_uvw - vel * fluid.params.x * fluid.inv_extent.xyz, vec3<f32>(0.0), vec3<f32>(1.0));
    let mapped = sample_uvw(src_map, back_uvw);
    textureStore(dst_map, coord, vec4<f32>(mapped.xyz, 0.0));
}
