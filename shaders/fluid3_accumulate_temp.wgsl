...
@group(0) @binding(2) var advected_temp: texture_3d<f32>;
@group(0) @binding(3) var current_temp: texture_3d<f32>;
@group(0) @binding(4) var delta_old: texture_3d<f32>;
@group(0) @binding(5) var delta_new: texture_storage_3d<r32float, write>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let advected = sample_scalar(advected_temp, map_pos, dims);
    let current = sample_scalar(current_temp, map_pos, dims);
    let delta = textureLoad(delta_old, coord, 0).x + (current - advected);
    textureStore(delta_new, coord, vec4<f32>(max(delta, 0.0), 0.0, 0.0, 0.0));
}
