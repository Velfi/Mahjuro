#!/usr/bin/env python3
"""
Generate a heightmap texture for the coin face using OpenAI's image API,
then post-process it into a clean displacement map (square, grayscale,
mid-gray background, raised features in white, recessed features in black).
...
PROMPT = (
    ...
    "Four traditional Chinese seal-script (篆書) characters are arranged "
    "around the central square hole, one at top, bottom, right, and left, "
    ...
    "The characters and the raised inner+outer rims are pure white. "
    "The flat coin field is mid-gray (#808080). "
    "The square center hole and the area outside the round coin are pure black. "
    ...
    "color tint, no metallic sheen. Just a clean tonal heightfield where "
    "lighter = higher and darker = lower. "
    "Slight age-worn surface variation in the field (very subtle) is fine, "
    "but characters must remain crisp and legible. "
    "Square 1:1 framing with the coin centered and filling ~90% of the frame. "
    "No text labels, no captions, no borders, no watermarks, no signatures."
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a Chinese cash-coin heightmap via the OpenAI image API"
    )
    parser.add_argument(
        "--name",
        type=str,
