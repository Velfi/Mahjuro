...
STYLE_PREFIX = (
    "A 192x256 pixel illustration of a single botanical motif, drawn in a "
    "traditional East Asian woodblock print / ink-wash style. The motif is "
    "rendered as a monochrome engraved relief — white lines and shapes on a "
    "pure transparent/black background. The composition is centered and "
    "vertically oriented to fit a tall rectangle. The style should evoke "
    "a carved mahjong tile face: clean, iconic, high contrast, no "
    "perspective. No text, no border, no background pattern. "
)

# Per-flower prompts describing the specific botanical motif.
FLOWER_PROMPTS: dict[str, str] = {
    "plum": (
        "A plum blossom (梅 méi) branch with 3-5 five-petaled blossoms and "
    ...
    "orchid": (
        "A wild orchid (蘭 lán) with slender arching leaves and a single "
    ...
    "chrysanthemum": (
        "A chrysanthemum (菊 jú) flower head with layered radiating petals, "
        "viewed from above. A few serrated leaves frame the bloom below. "
        "Dense but orderly petal arrangement, bold and round."
    ),
    "bamboo": (
        "A bamboo (竹 zhú) stalk segment with 2-3 nodes and several leaf "
