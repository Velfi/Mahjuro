...
use crate::render::wgpu_renderer::{GpuInstance, TextLabel, TooltipLayer};
use crate::ui::glossary;

// ── Data types ───────────────────────────────────────────────────────────

/// A hoverable glossary-term region on screen.
struct WordRegion {
    term: &'static str,
    rect: [f32; 4],
}
...
struct TooltipEntry {
    ...
    rect: [f32; 4],
    /// The word region that spawned this tooltip (part of its containment zone).
    anchor_rect: [f32; 4],
...
impl TooltipState {
    ...
    /// Run every frame.  Returns one [`TooltipLayer`] per tooltip in the chain
    /// so the renderer can draw each layer's quads+text before the next,
    /// giving child tooltips proper z-ordering over their parents.
    pub fn update_and_draw(
        ...
        let base_regions = regions_for_label_refs(&font, &non_button_labels, &[]);
        ...
        // ── Step 3: check for a hovered glossary term at the active depth ─
        let hovered: Option<(&'static str, [f32; 4])> = if self.chain.is_empty() {
        ...
        if let Some((term, word_rect)) = hovered {
            let already_open = self
                ...
                .unwrap_or(false);
            if !already_open {
                let exclude: Vec<&str> = self.chain.iter().map(|e| e.term).collect();
                let entry =
                    build_tooltip(&font, term, word_rect, scale, window_w, window_h, &exclude);
                self.chain.push(entry);
            }
        }

        // ── Step 5: render — one layer per tooltip for proper z-ordering ─
        self.chain
            .iter()
            .map(|entry| {
                let mut insts = Vec::new();
                let mut labels = Vec::new();
                draw_tooltip(entry, &mut insts, &mut labels);
                TooltipLayer {
                    instances: insts,
                    labels,
                }
...
fn hovered_region(cursor: (f32, f32), regions: &[WordRegion]) -> Option<(&'static str, [f32; 4])> {
    regions
        .iter()
...
fn build_tooltip(
    ...
    let entry = glossary::lookup(term).expect("term must be in glossary");
    ...
    let lines = wrap_text(font, entry.description, font_px, content_w);
...
fn draw_tooltip(entry: &TooltipEntry, insts: &mut Vec<GpuInstance>, labels: &mut Vec<TextLabel>) {
    ...
    // Cyan underlines for hoverable glossary terms in the description.
