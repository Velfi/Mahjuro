...
pub fn show_crash_report() {
    let log = CRASH_LOG
        ...
        .unwrap_or_else(|| "Mahjuro crashed (no details captured)".into());
...
fn show_crash_dialog(path: &std::path::Path, clipboard_ok: bool) {
    ...
    let script = r#"set msg to "Mahjuro has crashed unexpectedly." & return & return & "CLIP_MSG" & return & return & "Saved to:" & return & "LOG_PATH"
display alert "Mahjuro Crash" message msg as critical buttons {"Open Log", "OK"} default button "OK"
...
fn show_crash_dialog(path: &std::path::Path, clipboard_ok: bool) {
    ...
    eprintln!("Mahjuro crashed. Log saved to: {}{clip}", path.display());
