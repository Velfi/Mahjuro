...
pub struct RunChronicle {
    ...
    /// Total gold gained during the run (not ending balance).
    #[serde(default)]
    pub gold_earned: u32,
    #[serde(default)]
...
impl RunChronicle {
    ...
    pub fn note_gold_earned(&mut self, delta: i32) {
        if delta > 0 {
            self.gold_earned = self.gold_earned.saturating_add(delta as u32);
        }
    }
