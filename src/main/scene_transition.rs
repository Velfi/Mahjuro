...
impl From<&Scene> for SceneTag {
    fn from(scene: &Scene) -> Self {
        match scene {
            ...
            Scene::TextShadowLab(_) => SceneTag::TextShadowLab,
            Scene::TileAnchorLab(_) => SceneTag::TileAnchorLab,
            Scene::TileStressLab(_) => SceneTag::TileStressLab,
...
pub(crate) fn apply_post_scene_transition_effects(ctx: PostSceneTransitionCtx<'_>) {
    if let Some(r) = ctx.renderer {
        ...
        match ctx.to {
            ...
            SceneTag::Shop => {
                crate::asset_path::prefetch_gameplay_bulk_pack_once();
                ...
                crate::render::room_preload::advance_hub_cpu_prefetch_chain(false);
            }
            SceneTag::Hallway => {
                r.prefetch_room_chain_next(crate::render::room_preload::RoomSceneChain::Gameplay);
            }
    ...
    if ctx.to == SceneTag::MainMenu {
        crate::render::loader_pool::submit_pack_mount(|| {
            crate::asset_path::prefetch_lazy_packs();
        });
    }
