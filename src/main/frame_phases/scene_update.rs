...
pub fn run(app: &mut App, locals: &mut FrameLocals) {
    ...
    let tutorial_eligible = app.progress.runs_completed == 0 && !app.progress.tutorial_completed;
    ...
    let stairway_tile_pick = matches!(&app.scene, Scene::Stairway(s) if s.wants_hand_tile_pick());
    ...
    locals.update_result = if app.overlay_stack.is_empty() {
        app.scene.update(UpdateCtx {
            ...
            scroll_lines,
            tutorial_eligible: app.progress.runs_completed == 0 && !app.progress.tutorial_completed,
