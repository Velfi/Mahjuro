...
use crate::scene_transition::{
    ...
    apply_post_scene_transition_effects, sync_music_for_scene,
};
use crate::scenes;
...
pub fn run(app: &mut App, shell: &mut SdlShell, locals: &mut FrameLocals) {
    ...
    app.dispatch_rumble_lab_ops(
        shell,
        locals.now,
        std::mem::take(&mut locals.rumble_lab_ops),
    );
    ...
    if gameplay_cash_in_hold && let Some(input) = app.input.as_ref() {
        let progress = match &app.scene {
            Scene::Gameplay(g) => {
                ...
                g.cash_in_hold_progress(locals.now, trigger_enabled)
                    .unwrap_or(0.0)
            }
            _ => 0.0,
        };
