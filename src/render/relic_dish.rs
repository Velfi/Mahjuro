...
/// Build the dish mesh: a wide low box. Local units span roughly
/// -0.5..0.5 on each axis (same convention as the box) so callers can scale
/// it independently in x/y/z to choose the dish footprint and rim height.
pub fn build_dish_mesh() -> MeshCpu {
    let mut mesh = build_unit_box_mesh();
    // Tint to a darker brass — matches the Midnight Gold palette and reads
    // as a recessed metal tray under candlelight.
    mesh.default_material = MaterialParams {
        kind: MaterialKind::Plain,
