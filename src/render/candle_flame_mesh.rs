...
const HEIGHT_SEGMENTS: usize = 14;

/// Radius at normalized height `t` ∈ [0, 1] (0 = wick, 1 = tip).
fn flame_radius_at(t: f32) -> f32 {
    let t = t.clamp(0.0, 1.0);
    // Ease in from the wick so the bottom cap reads as a round dome, not a
    // vertical cylinder slice.
    let t = t * t * (3.0 - 2.0 * t);
    let belly = (1.0 - (t - 0.22).powi(2) * 14.0).max(0.0);
    0.10 + 0.36 * (1.0 - t).powf(0.48) + 0.10 * belly
}
...
pub fn build_candle_flame_volume_mesh() -> MeshCpu {
    ...
    let mut vertices: Vec<Vertex3dTex> =
        Vec::with_capacity((n_h + 1) * (n_rad + 1) + 2);
    ...
    // Bottom cap — wick-center fan so the flame base is a round dome, not an
    // open ring (which read as a flat disk from the side).
    ...
    for ri in 0..n_rad {
        ...
        let n0 = [c0, -0.55, s0];
        let n1 = [c1, -0.55, s1];
        let len0 = (n0[0] * n0[0] + n0[1] * n0[1] + n0[2] * n0[2]).sqrt().max(1e-6);
        let len1 = (n1[0] * n1[0] + n1[1] * n1[1] + n1[2] * n1[2]).sqrt().max(1e-6);
        let ring0 = ri as u32;
