...
//! [`crate::render::room_env_gltf::skip_room_env_authoring_mesh_node_name`].
...
/// Doc→world height scale for [`main_menu.glb`](../../../assets/3d/main_menu.glb) vs [`crate::render::room_glb::SHOP_ENV_HEIGHT_SCALE`].
///
...
fn ensure_main_menu_glb_loaded() {
    ...
    let ready = if let Some(file) = crate::asset_path::get("3d/main_menu.glb") {
        match load_main_menu_glb_from_bytes(&file.data) {
            ...
            Err(e) => {
                log::error!("main_menu.glb failed to load: {e:#}");
                None
            }
        }
    } else {
        log::debug!("main_menu.glb not embedded — main menu draws on black");
        None
    };
    ...
    with_main_menu_glb_cpu(|opt| {
        opt.map(|cpu| {
            crate::render::room_gltf_punctual::embedded_point_lights_runtime(
                ...
                env_h,
                tune,
                crate::render::room_gltf_punctual::RoomPunctualProfile::MainMenu,
                "main_menu.glb",
            )
        })
        .unwrap_or_default()
