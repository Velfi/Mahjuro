...
//!
//! Spawns drops throughout a padded room AABB, integrates in world −Z with wind,
...
use crate::render::particles::{ParticleSystem, RainCollider, RainSpawnVolume};
...
impl RainField {
    ...
    pub fn update(
        ...
        if !tuning.field_active() {
            self.particles.clear_world_drops();
        ...
        let pool = tuning.field.pool_size.round().max(1.0) as usize;
        let spawn_rate = tuning.field.spawn_rate.max(0.0) * speed.max(0.25);
        self.spawn_accum += spawn_rate * dt;
        while self.spawn_accum >= 1.0 && self.particles.world_drop_count() < pool {
            self.spawn_accum -= 1.0;
            self.particles.spawn_world_drop(
                volume.random_pos(),
                Vec3::ZERO,
                tuning.field.drop_color,
                tuning.field.streak_len_px,
            );
        }
    ...
    pub fn push_quads(&self, frame: &mut UiFrame, cam: &CameraParams, window_w: f32, window_h: f32) {
        ...
        for (rect, color) in self
            .particles
            .instances_world(cam, window_w, window_h)
            .into_iter()
