...
//! before mult lines** (then yen), so we reorder without changing the final total:
...
pub(crate) fn reorder_steps_chips_then_mult_then_yen(steps: &mut Vec<ScoreStep>, base_chips: i32) {
    #[derive(Clone)]
    struct Delta {
        source: String,
        kind: StepKind,
