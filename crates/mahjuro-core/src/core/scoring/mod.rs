//! Score hands as **Fu × Han**, Balatro-style.
//!
//! Each scored hand accumulates two parallel running totals:
//!
//! * **Fu** — additive. Tiles contribute their rank value (honors flat 12),
//!   then yaku, dora, and Fu-flavored relics pile on more.
//! * **Han** — multiplicative axis, but built additively (`+N Han`) so it
//!   stacks fast and predictably. Yaku and "explosive" relics live here.
//!
//! Final score = `final_fu × final_han` (floored).
//!
...
//! * [`pre_yaku_layer`] — meld-linked Fu/Han and early relic effects before Dora/yaku.
//! * [`dora_yaku_layer`] — Dora and yaku Fu/Han lines.
//! * [`relic_mult_layer`] — post-yaku relic Han and late relic Fu (one audit surface).
//! * [`effective_relic`] — Mirror Tile / Shadow Hand resolved once per score.
//! * [`presentation`] — optional regrouping of steps for the cascade (Fu before Han).

mod dora_yaku_layer;
pub use dora_yaku_layer::DORA_FU_PER_TILE;
pub mod effective_relic;
pub use effective_relic::EffectiveRelics;
mod layer_input;
mod pipeline;
mod pre_yaku_layer;
pub use pre_yaku_layer::{
    EVEN_KEEL_FU_PER_TILE, PLAIN_DEALING_FU_PER_SIMPLE_TILE, TRIPLET_BOOST_FU,
};
mod presentation;
mod push_steps;
...
/// Han before yaku or relic bonuses.
pub const SCORING_BASE_HAN: f64 = 1.0;
...
#[inline]
pub fn tile_is_debuffed(tile: &Tile, debuffs: &[crate::core::debuff::TileDebuff]) -> bool {
    debuffs.iter().any(|debuff| debuff.matches(tile))
}

pub(crate) fn combine(fu: i32, han: f64) -> u64 {
    ((fu.max(0) as f64) * han)
        .floor()
        .clamp(0.0, u64::MAX as f64) as u64
}

pub(crate) fn fmt_han(m: f64) -> String {
