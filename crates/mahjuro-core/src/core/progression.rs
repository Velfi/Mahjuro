...
/// Storeroom dwell time between `eyeball_travel` playback milestones.
pub const SHOP_EYEBALL_MILESTONE_SECS: f64 = 15.0 * 60.0;
...
impl PlayerProgress {
    ...
    /// Accumulate storeroom dwell time; returns newly crossed 15-minute milestones.
    pub fn accumulate_shop_storeroom_seconds(&mut self, dt_secs: f32) -> u32 {
        if !dt_secs.is_finite() || dt_secs <= 0.0 {
            return 0;
        }
        let prev = (self.shop_storeroom_seconds / SHOP_EYEBALL_MILESTONE_SECS).floor() as u32;
        self.shop_storeroom_seconds += dt_secs as f64;
        let next = (self.shop_storeroom_seconds / SHOP_EYEBALL_MILESTONE_SECS).floor() as u32;
...
impl PlayerProgress {
    ...
    pub fn apply_screenshot_collection_demo(
        ...
        for &kind in OrdealKind::ALL {
            *self.ordeal_times_encountered.entry(kind).or_insert(0) += 5;
            *self.ordeal_times_defeated.entry(kind).or_insert(0) += 2;
        }
...
mod tests {
    ...
    #[test]
    fn shop_storeroom_seconds_crosses_fifteen_minute_milestones() {
        let mut p = PlayerProgress::new();
        assert_eq!(p.accumulate_shop_storeroom_seconds(899.0), 0);
        assert_eq!(p.accumulate_shop_storeroom_seconds(1.0), 1);
        assert_eq!(p.accumulate_shop_storeroom_seconds(899.0), 0);
        assert_eq!(p.accumulate_shop_storeroom_seconds(1.0), 1);
        assert!(p.shop_storeroom_seconds >= SHOP_EYEBALL_MILESTONE_SECS * 2.0);
    }
}
