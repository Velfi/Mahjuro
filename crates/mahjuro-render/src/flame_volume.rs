//! Volumetric candle flames — one instance per [`FlameEmitter`].
//!
//! Scenes push emitters (wick anchor, scale, wind bias, brightness, phase); fast
//! brightness flicker runs in `shaders/flame.wgsl`. CPU-side [`flame_flicker_multiplier_amp`]
//! remains for candle punctual lights only.
...
pub struct FlameEmitter {
    ...
    pub scale: f32,
    /// Steady wind bias in world XY (global tuning bias is added in `flame.wgsl`).
    pub wind: Vec2,
    /// Scene brightness before fast flicker (light ramp, lamp flicker, …).
    pub brightness: f32,
...
impl GpuFlameInstance {
    #[inline]
    pub fn from_emitter(e: &FlameEmitter) -> Self {
        Self {
            anchor: e.wick_world.to_array(),
            wind_bias_x: e.wind.x,
            scale: e.scale,
            phase: e.phase,
            brightness: e.brightness,
            wind_bias_y: e.wind.y,
        }
    }
}

/// Fast smoothed 1D noise (random-walk steps). `seed` and `rate_hz` match `flame.wgsl`.
#[inline]
...
/// Candle punctual-light flicker — keep in sync with `flame_brightness_flicker` in `flame.wgsl`.
#[inline]
...
pub fn fill_gpu_instances(emitters: &[FlameEmitter], out: &mut Vec<GpuFlameInstance>) -> usize {
    out.clear();
    out.extend(emitters.iter().map(GpuFlameInstance::from_emitter));
    out.len()
}
