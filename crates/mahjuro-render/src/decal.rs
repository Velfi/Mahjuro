...
fn blit_flower_fallback(dst: &mut [u8], dst_w: u32, dst_h: u32, rank: u8) {
    ...
    if let Some(font_data) = mahjuro_assets::asset_path::get("NotoSansSC-Regular.ttf")
        .or_else(|| mahjuro_assets::asset_path::get("font.ttf"))
        && let Ok(font) =
...
/// demerit minimization ([`crate::text_wrap::break_units_kp`]). Words are
...
fn wrap_lines(
    ...
    use crate::text_wrap::{TextBreakUnit, break_units_kp};
    ...
    CACHE
        .get_or_init(|| {
            ...
            for path in candidates {
                if let Some(file) = mahjuro_assets::asset_path::get(path) {
...
pub fn load_mono_font() -> Option<&'static fontdue::Font> {
    ...
    CACHE
        .get_or_init(|| {
            ...
            mahjuro_assets::asset_path::get(path).and_then(|file| {
