//! Fixed-axis rotations for meshes in [`crate::world_space`].
//!
//! # World space (single convention)
//!
//! Everything ultimately uses [`crate::world_space`]: right-handed **Z-up**,
//! table in the **XY** plane, [`crate::world_space::pixel_to_world`] for layout → world.
//! Helpers here map **asset-local** axes into that frame — prefer composing these instead of ad‑hoc
//! basis matrices in renderers.
//!
//! # Units
//!
//! - **Degrees** — parameters and [`crate::draw_cmd`] fields whose names end in `_deg`.
...
/// Mahjong tile mesh from [`crate::tile_glb`] after `normalize_mesh` (which may rotate
...
/// **`Rx·Ry·Rz`** from degrees — convenience for filling [`crate::draw_cmd::Object3d::rotation`].
#[inline]
