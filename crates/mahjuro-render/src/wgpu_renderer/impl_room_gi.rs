...
pub(crate) struct RoomGiCaptureStaging {
    buffer: wgpu::Buffer,
    byte_len: u64,
    meta: crate::room_gi_bake::RoomGiBake,
}

impl WgpuRenderer {
    /// Queue a one-shot GPU readback of probe SH after the next dynamic GI compute
    /// (`mahjuro bake-room`).
    pub fn request_room_gi_capture(&mut self, room: crate::room_gi_bake::RoomGiRoom) {
        self.room_gi_capture_pending = Some(room);
        self.room_gi_captured = None;
        self.room_gi_capture_meta = None;
    }

    pub fn take_room_gi_capture(&mut self) -> Option<crate::room_gi_bake::RoomGiBake> {
    ...
    ) -> anyhow::Result<crate::room_gi_bake::RoomGiBake> {
