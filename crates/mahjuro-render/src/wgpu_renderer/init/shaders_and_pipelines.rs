...
pub(super) struct ShadersAndPipelinesInit {
    ...
    pub lit_mesh_spot_frame_bind_group: wgpu::BindGroup,
    ...
    pub tile_frame_uniform_buffer: wgpu::Buffer,
    pub tile_3d_instance_buffer: wgpu::Buffer,
    pub tile_shadow_instance_buffer: wgpu::Buffer,
    pub tile_shadow_frame_uniform_buffer: wgpu::Buffer,
...
pub(super) fn init_shaders_and_pipelines(
    ...
    let tile_shadow_instance_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("tile-shadow-instances"),
        size: (std::mem::size_of::<TileShadowInstance>() as u64)
            * (super::super::MAX_SHOWCASE_TILE_SLOTS as u64),
        usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
        mapped_at_creation: false,
    });
    let tile_shadow_frame_uniform_buffer =
        device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("tile-shadow-frame"),
    ...
    let tile_shadow_frame_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("tile-shadow-frame-bg"),
        layout: &shadow_caster_layout,
        entries: &[wgpu::BindGroupEntry {
            binding: 0,
            resource: tile_shadow_frame_uniform_buffer.as_entire_binding(),
        }],
    });
    ...
    //              group(1) = flame view globals (view_proj, view_pos).
    //
    // Lit_mesh spot+frame layout (group 3): created before `lit_mesh_pl`
    // because the pipeline references it; unrelated to the flame pipeline.
    let lit_mesh_spot_frame_layout = create_lit_mesh_spot_frame_layout(&device);
    ...
    // view_pos for view-relative tricks later). The lit-mesh frame
    ...
    let tile_3d_vertex_buffers = [tile_vertex_layout.clone(), tile_3d_instance_vertex_layout];
     ...
     -> wgpu::RenderPipeline {
        device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState {
                ...
                entry_point: Some("vs_main"),
                compilation_options: wgpu::PipelineCompilationOptions::default(),
                buffers: &tile_3d_vertex_buffers,
            },
            fragment: Some(wgpu::FragmentState {
                module: &tile_shader,
                entry_point: Some("fs_main"),
    ...
    let lit_mesh_pl = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        ...
        bind_group_layouts: &[
            ...
            Some(&point_lights_layout),
            Some(&shadow_sample_layout),
            Some(&lit_mesh_spot_frame_layout),
        ],
        immediate_size: 0,
    });
    let lit_mesh_frame_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("lit-mesh-frame-uniform"),
        contents: bytemuck::bytes_of(&LitMeshFrameGlobals {
            view_pos: [0.0; 4],
            hdr_tonemap: [0.0; 4],
            shop_punctual: [0.0; 4],
        }),
    ...
    let lit_mesh_spot_frame_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            ...
            wgpu::BindGroupEntry {
                binding: 1,
                resource: lit_mesh_frame_buffer.as_entire_binding(),
            },
        ],
    });
    ...
    ShadersAndPipelinesInit {
        ...
        tile_frame_uniform_buffer,
        tile_3d_instance_buffer,
        tile_shadow_instance_buffer,
        tile_shadow_frame_uniform_buffer,
