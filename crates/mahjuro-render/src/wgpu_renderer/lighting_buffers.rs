...
impl TileOccludersBuf {
    ...
    pub(crate) fn from_room_collision_meshes(
        model: glam::Mat4,
        meshes: &[crate::room_env_gltf::RoomCollisionMesh],
    ) -> Self {
...
fn room_collision_mesh_world_aabb(
    mesh: &crate::room_env_gltf::RoomCollisionMesh,
    model: glam::Mat4,
) -> (glam::Vec3, glam::Vec3) {
...
/// [`crate::draw_cmd::SceneLighting`]; the renderer translates
...
/// [`crate::draw_cmd::UiFrame::spot_lights`]; the renderer translates
...
impl SpotLightsBuf {
    ...
    /// [`crate::world_space::world_on_camera_ray_plane_z`] when `pos_cam`
    ...
    pub(crate) fn from_lights(
        ...
        screen_w: f32,
        screen_h: f32,
        pos_cam: Option<&crate::draw_cmd::CameraParams>,
    ) -> Self {
        ...
        for (i, l) in src.iter().take(n).enumerate() {
            let p = if let Some(cam) = pos_cam {
                crate::world_space::world_on_camera_ray_plane_z(
