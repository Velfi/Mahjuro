...
- [TPOS2 — tile pack opening #2](docs/agents/tpos2-art-direction.md) — art direction for the alternate pack celebration (`Arrival` → `Anticipation` → `Unseal` → `Deal` → `Settled`); parallel to [`src/scenes/showcase/tile_pack.rs`](src/scenes/showcase/tile_pack.rs).