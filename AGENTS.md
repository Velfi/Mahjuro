...
- **Asset packs** — `build.rs` runs `tools/bake_assets/bake_assets.py` into the Cargo output dir; runtime: `src/asset_sources.rs`, `tools/bake_assets/README.md`. Ships `mahjuro-pack-{shared,gameplay,scene-main_menu,music}.zip` + manifest. Override with `MAHJURO_ASSETS` / `MAHJURO_SKIP_ASSET_BAKE`.
