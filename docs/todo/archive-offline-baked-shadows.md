...
3. **Bake pass:** Only caster nodes draw in `mahjuro bake-room-shadows archive` (restore [`archive_prim_casts_room_shadow`](../../src/render/archive_glb.rs) to cubby-only or explicit caster list). Rebake after every `archive.glb` change; patch `mahjuro-pack-gameplay.zip` or ship via loose assets in dev.
